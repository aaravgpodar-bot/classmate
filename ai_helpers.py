...
def _load_env_file():
    """Load OPENAI_* keys from a local .env file if they are not already set.

    Keeps the API key server-side only. The .env file is gitignored, so the
    key never lands in committed or frontend code.
    """
...
def ai_study_coach(subject, stuck_on):
    """AI Study Coach: returns an encouraging message + a doable 3-5 step study
    plan generated by gpt-5.4-nano (OpenAI Responses API).

    Returns a dict: {"encouragement": str, "steps": [str, ...]}.
    """
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        raise RuntimeError("OPENAI_API_KEY is required for the AI Study Coach.")

    model = os.environ.get("OPENAI_MODEL", "gpt-5.4-nano")
    system = (
        "You are ClassMate's Study Coach, a warm, practical study mentor for a "
        "school student. You give short, kind encouragement and a concrete, "
        "doable study plan. Keep everything school-safe and age-appropriate."
    )
    user = (
        f"Subject: {subject}\n"
        f"What the student is stuck on: {stuck_on}\n\n"
        "Reply with ONLY JSON in this exact shape:\n"
        '{"encouragement":"string (1-2 warm sentences, second person)",'
        '"steps":["short concrete actionable step", "..."]}.\n'
        "Give between 3 and 5 steps. Each step must be a single concrete action "
        "the student can actually do today (not vague advice). Order them so the "
        "student can follow them top to bottom."
    )
    ...
    try:
        ...
        # Fallback: call the Responses REST API directly with urllib, matching the
        # rest of this file's style, so the feature still works without the SDK.
        raw = _study_coach_via_rest(api_key, model, system, user)
    except Exception:
        raise RuntimeError("OpenAI Study Coach request failed.")

    return _normalize_study_coach(raw, subject)


def _study_coach_via_rest(api_key, model, system, user):
    payload = {
        "model": model,
        "input": [
            {"role": "system", "content": system},
    ...
    if text:
        return text
    # Walk output[].content[].text if output_text is not present.
    chunks = []
...
def _normalize_study_coach(raw, subject):
    text = (raw or "").strip()
    parsed = {}
    if text:
        try:
    ...
    if not encouragement and not steps:
        raise RuntimeError("OpenAI Study Coach returned no usable plan.")
    if not encouragement:
        encouragement = f"You can absolutely make progress on {subject}. Let's take it one step at a time."
    if len(steps) < 3:
        raise RuntimeError("OpenAI Study Coach returned too few steps.")

    return {"encouragement": encouragement, "steps": steps}
